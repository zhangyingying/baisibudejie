...
static NSInteger pageThrough = 1;
@interface ZYYPictureThroughController ()
...
@implementation ZYYPictureThroughController
...
- (UIImageView *)imageView{
    if (_imageView == nil) {
        ...
        UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hidesImageViewThrough)];
...
- (void)afhttpRequest{
    ...
    [manager GET:kPictureThroughAPI parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)collectionLayout{
    ...
    [cancelButton addTarget:self action:@selector(handleCancelButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
...
//    ZYYHomeModel *model = [self.array objectAtIndex:indexPath.row];
    ...
    [cell.textImage addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapGestureThrough:)]];
    ...
    [cell.loveButton addTarget:self action:@selector(handleLoveButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [cell.hateButton addTarget:self action:@selector(handleHateButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [cell.forwardButton addTarget:self action:@selector(handleForwardButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [cell.commentButton addTarget:self action:@selector(handleCommentButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
    ...
    //收藏按钮
    [cell.collectionButton addTarget:self action:@selector(handleCollectionButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
    
    return cell;
}
- (void)handleTapGestureThrough:(UITapGestureRecognizer *)sender{
    ...
    [button addTarget:self action:@selector(handleDownloadButtonThrough:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:button];
}
- (void)handleDownloadButtonThrough:(UIButton *)sender{
    SingLetonMethod *sing = [SingLetonMethod shareSingleton];
    UIImageWriteToSavedPhotosAlbum([sing.singleImage image], nil, nil,nil);
}
- (void)hidesImageViewThrough{
    [UIView animateWithDuration:0.5 animations:^{
...
- (void)handleCollectionButtonThrough:(UIButton *)sender{
    UIView *view = [self.view viewWithTag:700];
    UIView *blackView = [self.view viewWithTag:800];
...
- (void)handleCancelButtonThrough:(UIButton *)sender{
...
- (void)handleLoveButtonThrough:(UIButton *)sender{
...
- (void)handleHateButtonThrough:(UIButton *)sender{
    ZYYPictureCell *cell = (ZYYPictureCell *)sender.superview.superview.superview;
    if (!sender.selected) {
...
- (void)handleForwardButtonThrough:(UIButton *)sender{
...
- (void)handleCommentButtonThrough:(UIButton *)sender{
